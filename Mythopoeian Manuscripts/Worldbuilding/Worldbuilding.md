---
tags: 
parent docs: "[[Mythopoeia]]"
sibling docs: 
child docs: 
date created: 2024-09-29T17:11:00
last updated: 2024-09-29T17:11:00
media: 
---
# [[Culture]]
A comprehensive documentation of all the different cultures that have lived and died in the Mythopoeian universe 
Just as Life is a force within this universe, Culture is too. 
### [[Terran Cultures]]
The force of Culture has a wide reach on the world of [[Terra]] as the Cultures of Terra have their roots in many species across the planet. from spirits to space dust. The main three Cultural forces come from three species groups, the [[Hominid]]s, the [[Draconic]]s and the [[Celephalopod]]s. 
- Flood basalt events are responsible for the [[Hominid]] legends of Hell
- the sinking of Atlantis, an empire formed mainly of marine life and some very hippy [[Hominid]]s, caused the ten plagues of the Nile
The Way a culture views itself in the real world reflects in the reality of the Cultures within the world of [[Terra]]

Cultural Development of [[Binding]] and [[Sign Manipulation]] throughout the history of Terra
The use of [[Signa]] follows a similar path to technology in energy usage, it starts out with using the energy from our bodies to do simple tasks and then it slowly transfers to using more elaborate tools, the complexity and energy usage of which only grows bigger.
Most simple sign uses internal energy sources to power the bindings, but more advanced spells are about using the environment and dedicated high density fuel sources for power as well as being more complex and precise 
writing

## [[Neboai Cultures]]
The Cultures of Neboai have a single origin within the [[High Glider]] species, the single pillar of language and societal structure, the one of many responsible for the world
# [[Location]]

# [[Magic]]

## [[Complex Plane]]

# [[Object]]

# [[Timeline]]
Like our universe, this was started by a "big bang", though the rate of expansion is equivalent to (insert formula here, i forgot it, its the shortest amount of time from two points in a gravitational field)
the universe ends when all patterns and energy are "expanded" to infinite simplicity

# [[Life]]

